...
Tenant resolution:
  ...
  - If not set (shared Lambda), derives the tenant AppConfig app name from
    tenant_code using the convention: {tenant_code}-{env_code}-{region_code}-article_curation_config
...
def _resolve_tenant_app_name(tenant_code, home_region=None):
    ...
    Convention: {tenant_code}-{env_code}-{region_code}-{home_region}-article_curation_config
    ...
    if home_region:
        return f"{tenant_code}-{env_code}-{region_code}-{home_region}-article_curation_config"
    return f"{tenant_code}-{env_code}-{region_code}-article_curation_config"
...
def load_config(tenant_code=None, home_region=None):
    ...
    Args:
        ...
        home_region: Optional. Tenant's home region for resource naming.
                     Used in AppConfig app name: {tenant}-{env}-{region}-{home_region}-article_curation_config
