...
import { S3BucketConstruct } from '@docforge/cdk-constructs/constructs/s3/s3-bucket-construct';

export interface ContentPipelineSharedStackProps extends cdk.StackProps {
  project: string;
  envName: string;
  envCode: string;
  regionCode: string;
...
export class ContentPipelineSharedStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props: ContentPipelineSharedStackProps) {
    ...
    // Key alias follows standard naming: shared-<envCode>-<regionCode>-docpipeline
    ...
    const kmsConstruct = new KmsKeyConstruct(this, 'PipelineEncryptionKey', {
      alias: `alias/${naming.prefixName('docpipeline')}`,
      description: 'Shared CMK for Document Processing pipeline resources (DynamoDB, SQS, S3)',
    ...
    if (vpcConfig.vpc_id && vpcConfig.subnet_ids?.length > 0) {
      ...
      const sg = new cdk.aws_ec2.SecurityGroup(this, 'LambdaSG', {
        vpc,
        securityGroupName: naming.prefixName('doc_lambda_sg'),
        description: 'Security group for Document Processing Lambda functions',
    ...
    const problemFinderBucketName = baseConfig.s3.content_generator_source_bucket?.name || '';
    ...
    // ── Shared AppConfig ─────────────────────────────────────────────────
    const sharedAppConfigName = naming.prefixName('doc_shared_config');
    ...
    const appConfigExtension = lambda.LayerVersion.fromLayerVersionArn(
      this, 'AppConfigExt',
      `arn:aws:lambda:${region}:000000000000:layer:AWS-AppConfig-Extension:${baseConfig.appconfig_extension_layer_version}`
    );

    const sharedLayer = new LambdaLayer(this, 'SharedLayer', {
      layerName: naming.prefixName('doc_shared_layer'),
    ...
    const lambdaRoleConstruct = new LambdaRoleConstruct(this, 'LambdaRole', {
      roleName: naming.prefixName('doc_lambda_exec_role'),
      description: 'Execution role for Document Processing pipeline Lambda functions with access to DynamoDB, S3, Bedrock, and S3 Vectors',
      policies: [
        { actions: ['s3vectors:ListVectors', 's3vectors:GetVectors', 's3vectors:QueryVectors', 's3vectors:PutVectors', 's3vectors:DeleteVectors'],
          resources: [`arn:aws:s3vectors:${region}:${accountId}:bucket/*-doc-vectors`, `arn:aws:s3vectors:${region}:${accountId}:bucket/*-doc-vectors/index/*`] },
        ...
        ...(baseConfig.change_detection?.glue?.database ? [
          ...
          { actions: ['s3:GetObject', 's3:PutObject', 's3:ListBucket'],
            resources: [
              `arn:aws:s3:::shared-${envCode}-${regionCode}-doc-pipeline`,
              `arn:aws:s3:::shared-${envCode}-${regionCode}-doc-pipeline/*`,
            ]
          },
        ] : []),
        { actions: ['secretsmanager:GetSecretValue'], resources: [`arn:aws:secretsmanager:${region}:${accountId}:secret:*-docpipeline-ticketsystem-api-key-*`] },
        ...
        // Invoke tenant-connector API Gateway (IAM auth) for TicketSystem config
        ...(baseConfig.ticketsystem?.tenant_connector_api_arn ? [{ actions: ['execute-api:Invoke'], resources: [baseConfig.ticketsystem.tenant_connector_api_arn] }] : []),
      ],
      tags: globalTags,
    });
    ...
    if (baseConfig.content_generator?.dynamodb_table) {
      lambdaRole.addToPrincipalPolicy(new cdk.aws_iam.PolicyStatement({
        actions: ['dynamodb:DescribeTable', 'dynamodb:Query', 'dynamodb:GetItem', 'dynamodb:Scan', 'dynamodb:BatchGetItem'],
        resources: [
          `arn:aws:dynamodb:${region}:${accountId}:table/${baseConfig.content_generator.dynamodb_table}`,
          `arn:aws:dynamodb:${region}:${accountId}:table/${baseConfig.content_generator.dynamodb_table}/index/*`,
        ],
      }));
    }
    ...
    const fnArticleEmbed = new StandardLambda(this, 'ArticleEmbedFn', {
      ...
      code: lambda.Code.fromAsset(path.resolve(__dirname, '../../functions/tagger')),
    ...
    const fnDedup = new StandardLambda(this, 'DedupFn', {
      ...
      runtime: lambda.Runtime.PYTHON_3_12,
      handler: 'handler.lambda_handler',
      code: lambda.Code.fromAsset(path.resolve(__dirname, '../../functions/scorer')),
    ...
    const sfnRoleConstruct = new LambdaRoleConstruct(this, 'SfnRole', {
      roleName: naming.prefixName('doc_sfn_exec_role'),
      description: 'Execution role for Document Processing Step Functions state machine',
      policies: [
    ...
    const fnFileEnumerator = new StandardLambda(this, 'FileEnumeratorFn', {
      ...
      runtime: lambda.Runtime.PYTHON_3_12,
      handler: 'handler.lambda_handler',
      code: lambda.Code.fromAsset(path.resolve(__dirname, '../../functions/scanner'), {
      ...
      environment: {
        ...
        CONTENT_GENERATOR_SOURCE_BUCKET: problemFinderBucketName,
        CONTENT_GENERATOR_TABLE: baseConfig.content_generator?.dynamodb_table || '',
        CONTENT_GENERATOR_GSI: baseConfig.content_generator?.gsi_name || '',
    ...
    const aslTemplate = fs.readFileSync(path.resolve(__dirname, '../../step-function/content-pipeline-pipeline.json'), 'utf-8');
    ...
    const sfnConstruct = new StepFunctionAslConstruct(this, 'CurationPipeline', {
      stateMachineName: naming.prefixName('content_pipeline_pipeline'),
    ...
    new EventBridgeRuleConstruct(this, 'PipelineSchedule', {
      ruleName: naming.prefixName('doc_pipeline_schedule'),
      description: 'Runs Document processing pipeline on schedule',
    ...
    const fnDispatcher = new StandardLambda(this, 'DispatcherFn', {
      ...
      runtime: lambda.Runtime.PYTHON_3_12,
      handler: 'handler.lambda_handler',
      code: lambda.Code.fromAsset(path.resolve(__dirname, '../../functions/orchestrator')),
    ...
    // ── SN Task Creator ──────────────────────────────────────────────────
    const fnTicketSubmitter = new StandardLambda(this, 'TicketSubmitterFn', {
      functionName: naming.prefixName('ticket_submitter'),
      description: 'Creates TicketSystem review tasks for generated articles - triggered by DynamoDB stream',
      runtime: lambda.Runtime.PYTHON_3_12,
      handler: 'handler.lambda_handler',
      code: lambda.Code.fromAsset(path.resolve(__dirname, '../../functions/reviewer')),
      role: lambdaRoleConstruct.role,
      memorySize: lc.ticket_submitter_memory,
      timeout: cdk.Duration.seconds(lc.ticket_submitter_timeout_seconds),
      environment: {
        ...lambdaEnv,
        S3_BUCKET: pipelineBucket.bucket.bucketName,
        ARTICLE_TABLE: articleTable.table.tableName,
        JOB_STATUS_TABLE: jobStatusTable.table.tableName,
        TENANT_CONNECTOR_API_URL: baseConfig.ticketsystem?.tenant_connector_api_url || '',
        TENANT_CONNECTOR_API_HOST: baseConfig.ticketsystem?.tenant_connector_api_host || '',
      },
      logRetentionDays: lc.log_retention_days,
      naming,
      ...vpcProps,
    });
    fnTicketSubmitter.function.addLayers(appConfigExtension, sharedLayer.layerVersion);
    articleTable.table.grantReadWriteData(fnTicketSubmitter.function);
    jobStatusTable.table.grantReadWriteData(fnTicketSubmitter.function);
    pipelineBucket.bucket.grantRead(fnTicketSubmitter.function);
    fnTicketSubmitter.function.addEventSource(new lambda_event_sources.DynamoEventSource(articleTable.table, {
      startingPosition: lambda.StartingPosition.LATEST, batchSize: lc.ticket_submitter_stream_batch_size, retryAttempts: 2,
    ...
    const fnWebhookHandler = new StandardLambda(this, 'WebhookHandlerFn', {
      ...
      runtime: lambda.Runtime.PYTHON_3_12,
      handler: 'handler.lambda_handler',
      code: lambda.Code.fromAsset(path.resolve(__dirname, '../../functions/callback_processor')),
    ...
    new BedrockGuardrailConstruct(this, 'SharedGuardrail', {
      guardrailName: naming.prefixName('content_pipeline_guardrail'),
      description: 'Content safety and contextual grounding for Document Processing Pipeline',
