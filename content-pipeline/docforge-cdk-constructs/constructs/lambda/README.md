...
```typescript
import { StandardLambda } from '@docforge/cdk-constructs';
...
```typescript
import { DockerLambda } from '@docforge/cdk-constructs';

const fn = new DockerLambda(this, 'ImageProcessor', {
  functionName: 'orgAlpha-d-use1-image-processor',
...
```typescript
import { LayeredLambda } from '@docforge/cdk-constructs';

const fn = new LayeredLambda(this, 'DataProcessor', {
  functionName: 'orgAlpha-d-use1-data-processor',
  runtime: lambda.Runtime.PYTHON_3_12,
  handler: 'index.handler',
...
In `docforge-infra` YAML config, Lambda functions support log group settings at both global and per-lambda level:
