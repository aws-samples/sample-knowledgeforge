...
Supports multiple tenants and three source systems: ITSM_KB, PROBLEM_FINDER_IC, PROBLEM_FINDER_RCA.
...
File Enumerator Lambda (REFACTORED + OPTIMIZED)
  ...
  │     • Single tenant: {"tenant_id": "acme"}
  │     • Multiple tenants: {"tenant_id": ["acme", "contoso"]}
  │     • All tenants: {} (default)
...
Source Bucket (data team's curated-unstructured-s3)
  ...
  e.g. tenant_partitioning/itsm/snow/kb_articles/acme/eu-west-1/KB0010576.json

Pipeline Bucket (pipeline working data)
  {tenant_id}/generated/*.json     Lean output payload for ServiceNow
...
infra/
  ...
  lib/article-curation-shared-stack.ts   Shared resources
  lib/article-curation-tenant-stack.ts   Per-tenant resources
...
lambda/
  ...
  dedup/                            Phase 2
  sn_task_creator/                  DynamoDB stream → ServiceNow
  webhook_handler/                  SQS approval queue → DynamoDB status update
  shared/python/                    Shared layer

step-function/article-curation-pipeline.json
