...
messages when a Knowledge Manager approves or rejects an article in ServiceNow.
...
def _delete_vector_from_index(tenant_id, article_id, home_region=None):
    ...
    """
    # Construct vector bucket name: {tenant_id}-{env_code}-{region_code}-{home_region}-kb-vectors
    if home_region:
        vector_bucket = f"{tenant_id}-{ENV_CODE}-{REGION_CODE}-{home_region}-kb-vectors"
        vector_index = f"{tenant_id}-{home_region}-kb-vectors"
    else:
        vector_bucket = f"{tenant_id}-{ENV_CODE}-{REGION_CODE}-kb-vectors"
        vector_index = f"{tenant_id}-kb-vectors"
