...
Supports tenant filtering via event payload:
  - {"tenant_id": "acme"} - process single tenant
  - {"tenant_id": ["acme", "contoso"]} - process multiple tenants
...
Environment variables:
  ...
  PROBLEM_FINDER_SOURCE_BUCKET - Problem Finder team's S3 bucket (IC/RCA articles)
  PROBLEM_FINDER_TABLE   - Problem Finder DynamoDB table for change detection
  PROBLEM_FINDER_GSI     - GSI name on Problem Finder table (tenant + generation_timestamp)
...
from tenant_discovery import discover_tenants
...
from batch_processor import create_tenant_batches, send_batches_to_sqs
from utils import tenant_s3_prefix

SOURCE_BUCKET = os.environ.get("SOURCE_BUCKET")
PIPELINE_BUCKET = os.environ.get("PIPELINE_BUCKET")
PROBLEM_FINDER_SOURCE_BUCKET = os.environ.get("PROBLEM_FINDER_SOURCE_BUCKET", "")
PROBLEM_FINDER_TABLE = os.environ.get("PROBLEM_FINDER_TABLE", "")
PROBLEM_FINDER_GSI = os.environ.get("PROBLEM_FINDER_GSI", "")
...
_problem_finder_resources_exist = None  # None = not checked yet


def _check_problem_finder_resources():
    ...
    """
    global _problem_finder_resources_exist
    if _problem_finder_resources_exist is not None:
        return _problem_finder_resources_exist

    if not PROBLEM_FINDER_TABLE or not PROBLEM_FINDER_GSI or not PROBLEM_FINDER_SOURCE_BUCKET:
        _problem_finder_resources_exist = False
        return False

    # Check DynamoDB table exists
    try:
        dynamodb.meta.client.describe_table(TableName=PROBLEM_FINDER_TABLE)
    except dynamodb.meta.client.exceptions.ResourceNotFoundException:
        log.info(
            "Problem Finder DynamoDB table does not exist, skipping",
            table=PROBLEM_FINDER_TABLE,
        )
        _problem_finder_resources_exist = False
        return False
    ...
    try:
        s3.head_bucket(Bucket=PROBLEM_FINDER_SOURCE_BUCKET)
    except Exception as e:
        error_code = getattr(e, "response", {}).get("Error", {}).get("Code", "")
        if error_code in ("404", "NoSuchBucket"):
            log.info(
                "Problem Finder S3 bucket does not exist, skipping",
                bucket=PROBLEM_FINDER_SOURCE_BUCKET,
            )
        else:
            log.warn(
    ...
    _problem_finder_resources_exist = True
    return True
...
def _query_problem_finder_articles(tenant_id, job_id, tenant_uuid=None):
    ...
    Args:
        tenant_id: Short tenant code (e.g. 'acme') — used for Problem Finder GSI query
    ...
    pf_table = dynamodb.Table(PROBLEM_FINDER_TABLE)
    ...
    try:
        resp = job_status_table.get_item(
            Key={
                "tenant_id": pk_value,
                "job_id": f"{tenant_id}_PROBLEM_FINDER_LAST_TIMESTAMP",
            }
        )
        item = resp.get("Item")
        if item:
    ...
    for doc in doc_records:
        ...
        source_system = (
            "PROBLEM_FINDER_RCA" if article_type == "RCA" else "PROBLEM_FINDER_IC"
        )

        # Read the actual article JSON from S3
        try:
            obj = s3.get_object(Bucket=PROBLEM_FINDER_SOURCE_BUCKET, Key=s3_key)
        ...
        if isinstance(article_data, dict):
            ...
            article_data["_source_s3_bucket"] = PROBLEM_FINDER_SOURCE_BUCKET
...
def process_all_tenants_parallel(tenants, job_id):
    ...
    log.info(
        ...
        problem_finder_enabled=_check_problem_finder_resources(),
    )

    def _process_one_tenant(tenant_id):
        # tenant_id here is the short code (e.g. 'acme')
        ...
        if _check_problem_finder_resources():
            try:
                pf_articles = _query_problem_finder_articles(tenant_id, job_id, tenant_uuid=tenant_uuid)
                if pf_articles:
                    # Group by source_system (IC vs RCA) for proper ingestion
                    pf_by_system = {}
                    for art in pf_articles:
                        ss = art.get("source_system", "PROBLEM_FINDER_IC")
...
def lambda_handler(event, context):
    ...
    log.info(
        ...
        problem_finder_source_bucket=PROBLEM_FINDER_SOURCE_BUCKET,
