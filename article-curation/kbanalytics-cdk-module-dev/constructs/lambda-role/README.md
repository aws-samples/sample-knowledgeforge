...
```typescript
import { LambdaRoleConstruct } from '@kbanalytics/cdk-constructs';

const lambdaRole = new LambdaRoleConstruct(this, 'MyLambdaRole', {
  roleName: 'acme-d-use1-order-processor-role',
  functionName: 'acme-d-use1-order-processor',
  kmsDecryptKeyArn: 'arn:aws:kms:us-east-1:123456789012:key/abc-123',
  policies: [
    {
...
```yaml
role:
  name: "acme-d-use1-order-processor-role"
  function_name: "acme-d-use1-order-processor"
  kms_decrypt_key_arn: "arn:aws:kms:us-east-1:123456789012:key/abc-123"
  policies:
    - actions: ["s3:GetObject"]
      resources: ["arn:aws:s3:::my-bucket/*"]
