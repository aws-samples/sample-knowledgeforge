...
import { ContentPipelineSharedStack } from '../lib/content-pipeline-shared-stack';
import { ContentPipelineTenantStack } from '../lib/content-pipeline-tenant-stack';
...
// Shared stack
const sharedStack = new ContentPipelineSharedStack(app, `shared-${envCode}-${regionCode}-docpipeline`, {
...
for (const tenant of tenants) {
  ...
  // Stack name format: <tenant>-<envCode>-<regionCode>-<connectRegion>-docpipeline
  // e.g. orgAlpha-d-euw1-us-east-1-docpipeline
  ...
  for (const stackEntry of stackEntries) {
    ...
    const stackName = `${tenant.tenantId}-${envCode}-${regionCode}-${connectRegion}-docpipeline`;

    new ContentPipelineTenantStack(app, stackName, {
      env: { account: accountId, region: regionName },
      project,
      envName,
      envCode,
