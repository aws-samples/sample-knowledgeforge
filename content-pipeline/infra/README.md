...
This is the infrastructure code for the article curation pipeline. It deploys everything needed to ingest TicketSystem KB articles, run them through classification, dedup, quality scoring, and enrichment via Bedrock LLMs, and produce enriched articles for knowledge manager review.
...
infra/
  ...
  lib/
    content-pipeline-stack.ts        # Main CDK stack
...
`lib/llmops-construct.ts` is a reusable CDK construct for LLM observability. Not tied to content-pipeline - any LLM use-case can import it.
