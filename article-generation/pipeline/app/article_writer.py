...
Builds the standardized kb_json envelope and writes to tenant-segregated S3 paths.
"""
import json
import logging
import uuid
...
def build_article_json(
    ...
    Returns (article_uuid, json_dict) with top-level 'kb_json' key.
    """
    article_uuid = str(uuid.uuid4())
    now_utc = datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S")
    return article_uuid, {
        "src_kb_article_id": article_uuid,
        "source_system": "PROBLEM_FINDER",
        ...
        "sys_created_by": "kbanalytics integration",
        "sys_domain": tenant,
        "language": "en",
        "valid_to": "",
        "active": "",
