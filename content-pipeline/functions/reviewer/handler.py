...
When an article's status changes to GENERATED, this Lambda:
  1. Loads tenant config from AppConfig (TicketSystem URL, API key ARN, etc.)
  ...
  6. Sends to TicketSystem catalog API:
...
TicketSystem config comes from tenant AppConfig (ticketsystem block):
...
from api_client import get_connector_config
...
def _mint_trust_key(tenant_code, caller_email, job_id, home_region=None, tenant_uuid=None):
    ...
    payload = {
        ...
        "ttl_seconds": 86400,
        "contact_id": f"content-pipeline-{job_id}",
        "session_id": job_id,
    }
...
def _get_sn_config(tenant_id, home_region=None):
    """Get TicketSystem config from tenant AppConfig."""
    config = _get_config(tenant_id, home_region)
    return config.get("ticketsystem", {})


def _map_to_sn(dw_field, tenant_id, home_region=None):
    """Map a data warehouse field name to TicketSystem field name."""
    config = _get_config(tenant_id, home_region)
...
def build_doc_payload_existing(payload, tenant_id=""):
    """Build KB JSON for existing ITSM_KB articles.
    Maps src_kb_article_id to TicketSystem field name via outbound mapping.
    """
...
def build_doc_payload_new(payload, article_record, tenant_id):
...
def send_to_ticketsystem(payload, article_record=None, tenant_id="", home_region=None, tenant_uuid=None):
    """Send article to TicketSystem catalog API.

    TicketSystem config comes from:
    ...
    if source_system == "ITSM_KB":
        doc_payload = build_doc_payload_existing(payload, tenant_id)
    else:
        doc_payload = build_doc_payload_new(payload, article_record, tenant_id)
    ...
    body = {
        "data": {
            ...
            "quantity": 1,
            "variables": {
                "doc_payload": json.dumps(doc_payload, ensure_ascii=False),
    ...
    headers = {
        ...
        "X-Platform-Contact-Id": f"content-pipeline-{payload.get('job_id', '')}",
        "X-Platform-Session-Id": payload.get("job_id", "content-pipeline-session"),
    ...
    logger.info(
        "REVIEW_REQUEST: Sending article to TicketSystem — url=%s, headers=%s, payload=%s",
        catalog_api_url,
    ...
    if resp.status not in (200, 201):
        ...
        logger.error(
            "TicketSystem API error — request details: "
        ...
        raise Exception(
            f"TicketSystem catalog API failed: HTTP {resp.status} - {resp.data.decode()[:500]}"
        )

    response_body = json.loads(resp.data.decode())
    logger.info(
        "REVIEW_REQUEST: TicketSystem response received — status=%d, response=%s",
        resp.status,
...
def update_article_status(tenant_id, article_id, api_response):
    """Update article_metadata with RITM details from TicketSystem API response."""
...
def process_article(tenant_code, article_id, kb_number, job_id, home_region=None, tenant_uuid=None, tenant_id=None):
    """Process a single article for TicketSystem submission.
    
    Args:
        tenant_code: Short code (e.g. 'orgBeta') — used for AppConfig, S3 paths
    ...
    try:
        ...
        api_response = send_to_ticketsystem(
        ...
        logger.info(f"TicketSystem RITM created: {ritm_number}")
        ...
        # If TicketSystem creation failed, release the reserved review slot
        if _get_review_limit(tenant_code, home_region) > 0:
            try:
                ...
                logger.info(
                    f"REVIEW_LIMIT: Released slot due to TicketSystem error for {tenant_id}"
                )
